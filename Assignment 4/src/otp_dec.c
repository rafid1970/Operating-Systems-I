/*
    ...
    Assignment 4 OTP
    otp_dec - Connects to respective Daemon program and asks it to start decryption.
*/
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
void serverOperations(int socketFD, char buffer[], char *cipherText, long cipherTextSize, char *key, long keySize);
...
int main(int argc, char *argv[]) {
    ...
    // Check user input
    if (argc != 4) {
        printf("Use Format - otp_dec cipherTextFile keyFile port\n");
        exit(1);
    }
    ...
    if (filePointer == NULL) {
        printf("Client Failure - Cannot open cipherText file!\n");
        exit(1);
    }

    // Get file size
    fseek(filePointer, 0, SEEK_END);
    long cipherTextSize = ftell(filePointer);
    fseek(filePointer, 0, SEEK_SET);

    // Allocate file buffer and read file data into buffer
    char *cipherText = (char*)malloc(cipherTextSize);
    fread(cipherText, cipherTextSize, 1, filePointer);
    ...
    if (checkStringCase(cipherText) == 1) {
    ...
    // Check if key is valid with cipherText
    ...
    // Initiate cipherText requests
    serverOperations(socketFD, buffer, cipherText, cipherTextSize, key, keySize);

    // Create space for decryption text, get/return response
    char *plainText = malloc(sizeof(char) * (cipherTextSize + 1));
    long plainTextSize = socketReadStream(socketFD, plainText, cipherTextSize);
...
void serverOperations(int socketFD, char buffer[], char *cipherText, long cipherTextSize, char *key, long keySize) {
    ...
    long sendingSize = htonl(cipherTextSize);
    ...
    fetchServerResponse(socketFD, buffer);
    socketWrite(socketFD, cipherText);
    fetchServerResponse(socketFD, buffer);
